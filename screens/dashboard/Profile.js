...
const Profile = (props) => {
  ...
  const pickImage = async () => {
    ...
    // console.log(result.assets[0], "resultresultresultresultresultresult");
  ...
  console.log(selectedImage, image, "KKKKKKKKKKKKKKKKKKKKK");
  // Edit profile starts
  ...
  var PEC = { ["PEC"]: pec };
  var HTTP = { ["HTTP"]: http };
  var logo = { ["profile_logo"]: selectedImage == "" ? "" : image };

  var pdoc = { ["pdocument"]: "k" };
  ...
  const editProfile = async () => {
    ...
    axios
      ...
      .then((response) => {
        // console.log("response in edit profile=" + JSON.stringify(response));
        setApiLoader(false);
        ...
        onPress: async () => {
          if (type.user_type === "Tanneries" && Platform.OS === "android") {
            ...
            if (subsciptionCancelled) {
              ToastAndroid.show(
                "Next Month Subscription Cancelled",
                ToastAndroid.LONG
              );
              // call delete api
              ...
              if (isDeleted) {
                ...
                setApiLoader(false);
                ToastAndroid.show(
                  "Can't Delete Account, Try In Some Time",
                  ToastAndroid.LONG
                );
              }
            } else {
  ...
  const turnOffNextMonthSubscription = async () => {
    ...
    const result = await axios.post(apiUrl + "/cancel-subscription-update", {
      subId: subId,
      status: true,
    });
  ...
  const deleteUser = async () => {
    let apiUrl =
      "https://www.hidetrade.eu/app/APIs/DeleteSingleUser/DeleteSingleUser.php";
        ...
        {apiLoader ? (
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View style={{ marginHorizontal: 10 }}>
              ...
              >
                <ButtonComp
                  title={"Insert/Update Leather Details"}
                  onPress={() =>
              ...
              >
                <View>
                  <ButtonForProfile title={"Update"} onPress={editProfile} />
                </View>
                <View>
                  <ButtonForLogout title={"Logout"} onPress={doLogout} />
                </View>
              </View>
              <View
                style={{
