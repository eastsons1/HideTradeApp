...
export default function Subscription(props) {
  ...
  const fetchData = async () => {
    ...
    console.log(subId, "subIdsubIdsubIdsubId");

    const subDetails = await axios.post(apiUrl + "/retrive-subsciption", {
      subId: subId,
    });
  ...
  const onTogglePressed = async () => {
    if (toggle) {
      Alert.alert(
        ...
        "Are You Sure You Want to Cancel Next Month Subscription ? ",
        [
          {
            text: "Yes",
            onPress: () => {
              axios
                .post(apiUrl + "/cancel-subscription-update", {
                  subId: subId,
                  status: true,
                })
                .then((res) => {
                  ToastAndroid.show(
                    "Next Month Subscription Canceled",
                    ToastAndroid.LONG
                  );
                  setToggle(false);
                })
      ...
      axios
        ...
        .then((res) => {
          ToastAndroid.show(
            "Next Month Subscription Enabled",
            ToastAndroid.LONG
          );
          setToggle(true);
        })
  ...
  const onCardDetailsChange = async () => {
    const result = await axios.post(apiUrl + "/checkout-session", {
      subId: subId,
      cusId: cusId,
    });
